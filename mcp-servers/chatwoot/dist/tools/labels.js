...
export function registerLabelTools({ server, client }) {
    server.tool('chatwoot_list_labels', 'List all labels in the Chatwoot account.', {}, async () => {
        try {
    ...
    server.tool('chatwoot_create_label', 'Create a new label in the Chatwoot account.', {
        ...
        try {
            ...
            if (args.show_on_sidebar !== undefined)
                body.show_on_sidebar = args.show_on_sidebar;
    ...
    server.tool('chatwoot_update_label', 'Update an existing label.', {
    ...
    server.tool('chatwoot_delete_label', 'Delete a label from the Chatwoot account.', {
