...
export function registerConversationTools({ server, client }) {
    server.tool('chatwoot_list_conversations', 'List conversations with optional filters for status, assignee type, inbox, team, labels, and pagination.', {
        ...
        try {
            ...
            if (args.inbox_id)
                params.inbox_id = String(args.inbox_id);
            if (args.team_id)
                params.team_id = String(args.team_id);
            if (args.labels)
    ...
    server.tool('chatwoot_create_conversation', 'Create a new conversation in a given inbox for a contact.', {
    ...
    server.tool('chatwoot_get_conversation', 'Get a single conversation by its ID.', {
    ...
    server.tool('chatwoot_update_conversation', 'Update a conversation (e.g. custom attributes, status).', {
    ...
    server.tool('chatwoot_filter_conversations', 'Filter conversations using advanced query filters.', {
    ...
    server.tool('chatwoot_toggle_conversation_status', 'Toggle the status of a conversation (open, resolved, pending, snoozed).', {
    ...
    server.tool('chatwoot_toggle_conversation_priority', 'Toggle the priority of a conversation (none, low, medium, high, urgent).', {
        conversation_id: z.number().describe('ID of the conversation'),
    ...
    server.tool('chatwoot_mute_conversation', 'Mute a conversation to stop receiving notifications.', {
    ...
    server.tool('chatwoot_unmute_conversation', 'Unmute a conversation to resume receiving notifications.', {
    ...
    server.tool('chatwoot_get_conversation_counts', 'Get conversation counts grouped by status and assignee type.', {
    ...
    server.tool('chatwoot_snooze_conversation', 'Snooze a conversation until a specified date/time.', {
    ...
    server.tool('chatwoot_get_conversation_activities', 'Get the activity/audit log for a conversation.', {
        conversation_id: z.number().describe('ID of the conversation'),
        page: z.number().optional().describe('Page number for pagination'),
    }, async (args) => {
        try {
