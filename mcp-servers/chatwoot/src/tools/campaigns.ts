...
export function registerCampaignTools({ server, client }: ToolContext): void {
  server.tool(
    'chatwoot_list_campaigns',
    'List all campaigns in the Chatwoot account.',
    {},
    async () => {
      try {
  ...
  server.tool(
    ...
    async (args) => {
      try {
        ...
        if (args.trigger_rules !== undefined) body.trigger_rules = args.trigger_rules;
