...
export function registerConversationLabelTools({ server, client }: ToolContext): void {
  server.tool(
    'chatwoot_get_conversation_labels',
    'Get all labels assigned to a conversation.',
    {
  ...
  server.tool(
    'chatwoot_set_conversation_labels',
    'Set labels on a conversation (replaces all existing labels).',
    {
      conversation_id: z.number().describe('ID of the conversation'),
