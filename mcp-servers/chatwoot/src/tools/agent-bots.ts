...
export function registerAgentBotTools({ server, client }: ToolContext): void {
  server.tool(
    'chatwoot_list_agent_bots',
    'List all agent bots in the Chatwoot account.',
    {},
    async () => {
      try {
  ...
  server.tool(
    ...
    async (args) => {
      try {
        ...
        if (args.bot_type !== undefined) body.bot_type = args.bot_type;
        if (args.bot_config !== undefined) body.bot_config = args.bot_config;
