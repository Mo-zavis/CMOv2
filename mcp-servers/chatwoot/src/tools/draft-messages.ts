...
export function registerDraftMessageTools({ server, client }: ToolContext): void {
  server.tool(
    'chatwoot_get_draft_message',
  ...
  server.tool(
    'chatwoot_update_draft_message',
    'Update or set the draft message for a conversation.',
    {
      conversation_id: z.number().describe('ID of the conversation'),
    ...
    async (args) => {
      try {
        ...
        if (args.reply_type !== undefined) body.reply_type = args.reply_type;
