...
export function registerContactNoteTools({ server, client }: ToolContext): void {
  server.tool(
    'chatwoot_list_contact_notes',
  ...
  server.tool(
    ...
    async (args) => {
      try {
        return ok(await client.app.put(
          `/contacts/${args.contact_id}/notes/${args.note_id}`,
