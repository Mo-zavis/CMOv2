...
export function registerMessageTools({ server, client }: ToolContext): void {
  server.tool(
  ...
  server.tool(
    ...
    async (args) => {
      try {
        ...
        if (args.message_type !== undefined) body.message_type = args.message_type;
  ...
  server.tool(
    ...
    async (args) => {
      try {
        return ok(await client.app.patch(
          `/conversations/${args.conversation_id}/messages/${args.message_id}`,
