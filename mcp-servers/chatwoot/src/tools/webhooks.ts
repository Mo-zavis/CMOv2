...
export function registerWebhookTools({ server, client }: ToolContext): void {
  server.tool(
    'chatwoot_list_webhooks',
    'List all webhooks configured in the Chatwoot account.',
    {},
    async () => {
      try {
  ...
  server.tool(
    ...
    async (args) => {
      try {
        ...
        if (args.subscriptions !== undefined) body.subscriptions = args.subscriptions;
