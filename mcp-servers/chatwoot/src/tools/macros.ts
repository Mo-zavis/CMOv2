...
export function registerMacroTools({ server, client }: ToolContext): void {
  server.tool(
    'chatwoot_list_macros',
    'List all macros in the Chatwoot account.',
    {},
    async () => {
      try {
  ...
  server.tool(
    'chatwoot_execute_macro',
    'Execute a macro on one or more conversations.',
    {
