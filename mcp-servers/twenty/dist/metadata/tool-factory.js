...
export function registerMetadataTools(server, client, resource) {
    // 1. metadata_list_{plural}
    server.tool(`metadata_list_${resource.plural}`, `List all ${resource.plural} (metadata). ${resource.description}`, {
        limit: limitSchema,
    ...
    server.tool(`metadata_get_${resource.singular}`, `Get a single ${resource.singular} by ID (metadata). ${resource.description}`, { id: idSchema }, async (args) => {
        try {
    ...
    server.tool(`metadata_create_${resource.singular}`, `Create a new ${resource.singular} (metadata). ${resource.description}`, { data: dataSchema }, async (args) => {
        try {
    ...
    server.tool(`metadata_update_${resource.singular}`, `Update an existing ${resource.singular} by ID (metadata).`, { id: idSchema, data: dataSchema }, async (args) => {
        try {
    ...
    server.tool(`metadata_delete_${resource.singular}`, `Delete a ${resource.singular} by ID (metadata).`, { id: idSchema }, async (args) => {
        try {
