...
function buildParams(args, keys) {
    const params = {};
...
export function registerObjectTools(server, client, obj) {
    // 1. list_{plural}
    server.tool(`list_${obj.plural}`, `List/search ${obj.plural}. ${obj.description}`, {
    ...
    // 2. get_{singular}
    server.tool(`get_${obj.singular}`, `Get a single ${obj.singular} by ID. ${obj.description}`, {
        id: s.idSchema,
        depth: s.depthSchema,
    }, async (args) => {
        try {
    ...
    // 3. create_{singular}
    server.tool(`create_${obj.singular}`, `Create a new ${obj.singular}. ${obj.description}`, {
        data: s.dataSchema,
        depth: s.depthSchema,
    ...
    // 4. update_{singular}
    server.tool(`update_${obj.singular}`, `Update a single ${obj.singular} by ID.`, {
        id: s.idSchema,
        data: s.dataSchema,
        depth: s.depthSchema,
    }, async (args) => {
    ...
    // 5. delete_{singular}
    server.tool(`delete_${obj.singular}`, `Delete a single ${obj.singular} by ID. Use soft_delete=true to allow restoration later.`, {
        id: s.idSchema,
    ...
    server.tool(`update_many_${obj.plural}`, `Update multiple ${obj.plural} matching a filter.`, {
    ...
    server.tool(`delete_many_${obj.plural}`, `Delete multiple ${obj.plural} matching a filter. Use soft_delete=true to allow restoration.`, {
        filter: s.requiredFilterSchema,
        soft_delete: s.softDeleteSchema,
    }, async (args) => {
        try {
    ...
    server.tool(`batch_create_${obj.plural}`, `Create multiple ${obj.plural} in one request (max 60).`, {
    ...
    server.tool(`find_duplicate_${obj.plural}`, `Find duplicate ${obj.plural}. Provide a record to match against.`, {
        data: s.dataSchema,
        depth: s.depthSchema,
    }, async (args) => {
        try {
    ...
    server.tool(`restore_${obj.singular}`, `Restore a soft-deleted ${obj.singular} by ID.`, {
        id: s.idSchema,
        depth: s.depthSchema,
    }, async (args) => {
        try {
    ...
    server.tool(`restore_many_${obj.plural}`, `Restore multiple soft-deleted ${obj.plural} matching a filter.`, {
        filter: s.requiredFilterSchema,
        depth: s.depthSchema,
    }, async (args) => {
        try {
    ...
    // 12. merge_{plural}
    server.tool(`merge_${obj.plural}`, `Merge duplicate ${obj.plural}. Provide primary record ID and IDs to merge into it.`, {
        ...
        try {
            const params = buildParams(args, ['depth']);
            return ok(await client.patch(`/rest/${obj.plural}/merge`, { primaryRecordId: args.primary_record_id, mergeRecordIds: args.merge_record_ids }, params));
        }
        catch (e) {
            return formatError(e);
        }
    ...
    // 13. group_by_{plural}
    server.tool(`group_by_${obj.plural}`, `Group ${obj.plural} by fields with optional aggregation.`, {
